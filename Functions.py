...
calib_df = pd.read_csv('calibration_real.csv',index_col = 0)
...
def lux_dis_combined(data_prepare1,data_prepare2,area_pos1,area_pos2):
    lux_ = {}
    lux_table_st1 = lux_table_status(dark1)
    lux_table_st2 = lux_table_status(dark2)
    for st_ in range(3):
...
#     lux3 = lux_to_distance(lux_table_st[3],area_dict)
    ...
    for ii in range(3):
        lux_t = lux_[ii+1] 
        if save:
            lux_t.to_csv('lux_'+str(ii+1)+'_final.csv',index=False)
...
#     temp_df[['x','y','lux']]
    ...
    return sym_y
def Board_field(sym_y,sym,smoothing):
    bk_board1 = Board_light(dark1,area_pos1[0],sym_y =sym_y,sym=sym)
    bk_board2 = Board_light(dark2,area_pos2[0],sym_y =sym_y,sym=sym)
    bk_board = pd.concat([bk_board1,bk_board2])
#     bk_board[bk_board.index!='2c':,'lux'] = bk_board.drop('2c').apply(lambda x: x['lux']/calibration.loc[x.name,'slope'] ,axis=1)
...
def convolution_evl2(lux_mode,template_bb,area_,delta=0.125,exclude_mode=None):
    ...
    error_dict = {}
    result_eval = []
    dark = Dark_model(delta=delta,kernel_size=10,mn=[-1,-1],mx=[11,11])
...
def mode_outlier(error_comb,g_thresh = 50):
    group_err = {}
...
def build_space_calibration(new_X,new_Y,smoothing=0.6,filter_size=5,data2=None):
    if data2 is None:
        data2 = pd.read_csv('Space_calibration.csv')
#     new_X,new_Y = np.meshgrid(np.arange(mn[0], mx[0]+delta, delta), np.arange(mn[1], mx[1]+delta, delta))
    xflat = np.dstack((new_X.flatten(),new_Y.flatten()))[0]
...
def plot_calibrate(error_comb):
    ...
    sns.scatterplot(data= error_comb,x='lux',y='diff',style='area',hue="area")#,markers=True, dashes=False)
    ...
    table.set_fontsize(12)
    table.scale(1.5, 1.5)  # may help
    sns.scatterplot(data= error_comb,x='lux',y='diff2',style='area',hue="area")#,markers=True, dashes=False)
    ...
    sns.histplot(data=error_comb, x="abs_diff2", log_scale=False)
...
def convolution_final(lux_mode,template_bb,area_,samples= 5,delta=0.25,modes_list=None,draw=False):
    ...
    result_eval = []
    global pos_df
    dark = Dark_model(delta=delta,kernel_size=10,mn=[-1,-1],mx=[11,12])
    if samples=='ALL':
